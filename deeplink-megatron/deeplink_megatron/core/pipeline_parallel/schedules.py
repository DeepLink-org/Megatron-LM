...
from deeplink_megatron.core.pipeline_parallel.offload import get_pipeline_offload_runtime
...
def forward_backward_no_pipelining(
    *,
    ...
    offload_runtime = get_pipeline_offload_runtime()
    if config.timers is not None:
    ...
    with no_sync_func():
        for i in range(num_microbatches - 1):
            with offload_runtime.forward_microbatch(
                phase="no_pipeline",
                virtual_microbatch_id=i,
                model_chunk_id=0,
            ):
            ...
            if not forward_only:
                with offload_runtime.backward_microbatch(
                    phase="no_pipeline",
                    virtual_microbatch_id=i,
                    model_chunk_id=0,
                ):
    ...
    last_microbatch_id = num_microbatches - 1
    with offload_runtime.forward_microbatch(
        phase="no_pipeline",
        virtual_microbatch_id=last_microbatch_id,
        model_chunk_id=0,
    ):
        output_tensor, num_tokens = forward_step(
            forward_step_func,
            ...
            current_microbatch=last_microbatch_id,
        )
    total_num_tokens += num_tokens
...
def forward_backward_pipelining_with_interleaving(
    *,
    ...
    def get_offload_key(virtual_microbatch_id, *, forward):
    ...
    def forward_step_helper(
        ...
        no_offload = (
            ...
            and model_chunk_id == num_model_chunks - 1
        )
        ...
        with offload_runtime.forward_microbatch(
            ...
            enabled=not no_offload,
            offload_key=get_offload_key(virtual_microbatch_id, forward=True),
        ):
            output_tensor, num_tokens = forward_step(
                forward_step_func,
    ...
    def backward_step_helper(virtual_microbatch_id):
        ...
        with offload_runtime.backward_microbatch(
            ...
            virtual_microbatch_id=virtual_microbatch_id,
            model_chunk_id=model_chunk_id,
            enabled=not no_reload,
            offload_key=get_offload_key(virtual_microbatch_id, forward=False),
        ):
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    for i in range(num_microbatches_remaining):
        ...
        with offload_runtime.forward_microbatch(
            phase="steady",
            virtual_microbatch_id=current_microbatch,
            model_chunk_id=0,
        ):
            output_tensor, num_tokens = forward_step(
                forward_step_func,
        ...
        if forward_only:
            ...
            with offload_runtime.backward_microbatch(
                phase="steady",
                virtual_microbatch_id=backward_microbatch_id,
                model_chunk_id=0,
            ):
                input_tensor_grad = backward_step(
                    input_tensor, output_tensor, output_tensor_grad, model_type, config
    ...
    if not forward_only:
        for i in range(num_warmup_microbatches):
            ...
            input_tensor = input_tensors.pop(0)
            output_tensor = output_tensors.pop(0)
            backward_microbatch_id = i + num_microbatches_remaining
