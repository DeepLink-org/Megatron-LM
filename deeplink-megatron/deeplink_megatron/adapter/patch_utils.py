"""Minimal clean-room patch helper used by deeplink_megatron.

The helper only supports the patching mode deeplink_megatron needs for Megatron-LM:
...
class Patch:
    ...
    def set_patch_func(
        ...
        if remove_origin_wrappers:
            raise NotImplementedError("deeplink_megatron patch helper does not strip existing wrappers")
    ...
    def apply_patch(self):
        ...
        owner_name, attr_name = _split_target(self.target_name)
...
class MegatronPatchesManager:
    ...
    def register_patch(
        ...
        **kwargs
        ...
        if create_dummy:
            raise NotImplementedError("deeplink_megatron patch helper requires existing Megatron symbols")
